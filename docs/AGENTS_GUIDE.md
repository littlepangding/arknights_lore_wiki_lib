...
- **Summary layer** at `kb_summaries/` (in git, optionally LLM-baked via `scripts/kb_summarize.py`):
  ...
  - `stages/<event_id>/<NN>.md` — per-`<章节>` summary (same 4-tag shape, ≤200 字 核心剧情), baked by `kb_summarize --stages`. The chapter-level retrieval layer: event → its stage one-liners → pick a chapter → pull raw `正文`. When populated, its `<关键人物>` also feed stage-granular `summary`-source char↔stage edges (see "Edge confidence" below). (Code shipped; the corpus-wide bake may not be run yet — check whether `kb_summaries/stages/` is populated.)
...
- **`summary`** — from the `<关键人物>` tag of a baked summary, each surface name resolved through the alias index; catches chars referred to only by a title/nickname a name-grep misses. Treated as `tier=named` for `--min-tier`. *Event-scoped* (`stage_idx` is `null`) where only an event summary is baked; where the per-`<章节>` summary is baked (`kb_summarize --stages` → `kb_summaries/stages/<event_id>/<NN>.md`), its `<关键人物>` add **stage-granular** `summary` edges (real `stage_idx`, picked up by `stage_chars`) and the event-scoped edge for that `(char, event)` is dropped. So whether a `summary` edge has a `stage_idx` depends on whether that event's `<章节>` summaries have been baked yet.
...
**For `博士` (player avatar / mainline protagonist), the protagonist dossier lives at `kb_curated/chars/ent_76be2e/{profile,archive,voice_about}.md`** and is reachable via `kb_query entity get ent_76be2e --section profile|archive|voice_about|all`. `entity resolve 博士` (or `Doctor` / `Dr.`) lands on the same id. `entity appearances ent_76be2e` lists every event where 博士 turns up in a baked summary's `<关键人物>`. The dossier is the only protagonist-specific surface — `char_id` / `card` / `cooccur` / `relations` remain operator-only by design.

**For "who does X mostly co-star with?", use `kb_query relations cooccur for <char_id>`.** Deterministic, derived from the WS-0 char↔stage edges — `co_stage_count` is the high-signal metric (both chars share an explicit stage), `co_event_count` is the wider net. Defaults to `--min-tier named` so lone `mentioned` participant edges don't drown the table. `cooccur top` lists the corpus-wide leaders; `cooccur between <a> <b>` is a fast yes/no for whether two chars overlap at all.

**For typed assertions like "is Amiya a Rhodes Island member?", use `kb_query relations for <entity_id>` and `relations between <a> <b>`.** Populated by the LLM bake at `scripts/kb_relations.py` (one call per operator over `profile.txt` + `archive.txt` + `voice.txt`; output: `kb_relations/chars/<char_id>.jsonl`; collated by `kb_build` into `data/kb/relations.jsonl`). Vocabulary: `member_of`, `ally_of`, `rival_of`, `family_of`, `mentor_of`, `subordinate_of`, `creator_of`, `identifies_as`, `origin_from` — plus any novel types the bake committed (flagged in bake warnings). Empty until you run the bake (`kb_relations --estimate` shows the cost first). A row whose `tail` is `null` is an ambiguous-tail assertion preserved with `tail_name` + `ambiguous_candidates` — the curator narrows it later via `entities_curated.jsonl`.
...
.venv/bin/python -m scripts.kb_query entity list [--type operator|npc|organization|location|group|protagonist|unknown]
.venv/bin/python -m scripts.kb_query entity get <entity_id> [--section profile|archive|voice_about|all] [--text]    # `--section` reads `kb_curated/chars/<entity_id>/` (non-operator dossier); v1 has 博士 (ent_76be2e) only
.venv/bin/python -m scripts.kb_query entity appearances <entity_id>   # summary-source appearance rows for a non-operator entity (curated NPC / protagonist / auto-seeded). Operators use `char appearances` instead.
