...
## P1b — Per-stage summary (`kb_summarize --stages`, committed to `kb_summaries/stages/<event_id>/<NN>.md`)

Same system prompt as P1. One call per `<章节>` chunk — always single-pass (no stage chunk approaches the multi-pass threshold). Same 4 output tags as P1 but scoped to the one chapter, with tighter length caps.

**User:**
```
以下是明日方舟某次活动一个章节的剧情原文。请基于原文输出该章节的导航摘要，仅供索引检索，不替代原文：
...
Required output tags (validated): same as P1 (`一句话概要`, `核心剧情`, `关键人物`, `场景标签`). Distinct from the P1 multi-pass per-stage *reduce* prompt above (`<章节概要>` / `<本章人物>`), which is an internal intermediate, not a committed artifact.
