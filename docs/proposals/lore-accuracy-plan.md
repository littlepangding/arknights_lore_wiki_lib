...
| P-C | Per-stage summaries (`kb_summarize --stages`, `kb_summaries/stages/`) | ≈ one bake (a touch less — per-chapter, no multipass) | chapter-level retrieval (the explicit ask); also upgrades WS-0's `summary`-source edges to stage granularity | |
