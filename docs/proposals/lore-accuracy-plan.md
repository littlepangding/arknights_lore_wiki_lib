...
| P-C | Per-stage summaries (`kb_summarize --stages`, `kb_summaries/stages/<event_id>/<NN>.md`) + the `<关键人物>`→stage-granular `summary`-edge upgrade in `indexer`/`kb_build` | ≈ one bake (a touch less — per-chapter, no multipass; ~1937 calls, ~13M tok by `--stages --estimate`) | chapter-level retrieval (the explicit ask); also makes `summary`-source char↔stage edges stage-scoped (no behaviour change until the bake produces those files) | **code shipped** (`kb-stage-summaries`); the LLM bake is the user's to run |
