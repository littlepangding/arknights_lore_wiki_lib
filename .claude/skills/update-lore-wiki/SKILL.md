...
This skill orchestrates a full update of the lore wiki when ArknightsGameData publishes new content. It assumes three sibling repos under `~/Claude/`:
...
```
cd ~/Claude/ArknightsGameData     && find . -name .DS_Store -delete && git pull --ff-only origin master
cd ~/Claude/arknights_lore_wiki   && find . -name .DS_Store -delete && git pull --ff-only origin update
cd ~/Claude/arknights_lore_wiki_lib && git status
```
...
```
cd ~/Claude/arknights_lore_wiki_lib
